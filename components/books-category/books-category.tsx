...
function BooksCategory({ slug }: { slug: string }) {
    ...
    useEffect(() => {
        const fetchBooks = async () => {
            setLoading(true)
            try {
                const response = await bookApi.getBooksByCategory(slug, page)
    ...
    if (!Array.isArray(books) || books.length < 1) return <div>No books available</div>
        ...
        <section className="pt-40">
            <div className='max-w-6xl mx-auto '>
                ...
                <div className='w-full flex flex-row items-center gap-2 px-1 sm:px-5 py-2 bg-gray-800 rounded-md'>
                    <h3 className="text-2xl">Danh sách truyện {slug}</h3>
                </div>
