...
export default function Profile() {
    const { email, displayName, avatar, updateUserProfile, createNewPassword } = useStore()
    ...
    async function uploadImage(image: any): Promise<string> {
        const storageReference = storageRef(storage, `avatars/${image?.name || 'image'}`)
        await uploadBytes(storageReference, image)
        return await getDownloadURL(storageReference)
    }
    async function handleUpdateProfile() {
        try {
            const newAvatarUrl = await uploadImage(imageFile)
            await updateUserProfile(newDisplayName, newAvatarUrl)
        } catch (error) {
    ...
    return (
        <section>
            <div className="max-w-6xl mx-auto px-4 sm:px-6 relative">
                ...
                <div className="relative pt-32 pb-10 md:pt-40 md:pb-5">
                    <div className="max-w-3xl mx-auto text-center pb-12 md:pb-16">
                        <div className='w-full flex flex-row items-center justify-center pb-4'>
                            <img className='w-40 h-40 items-center rounded-full' loading='lazy'
                                src={avatar || 'https://th.bing.com/th/id/OIP.SJGVqdRK426_7c-ZDGQTFwHaHa?w=1920&h=1920&rs=1&pid=ImgDetMain'} />
                        </div>
                        <h1 className="h1 mb-4" data-aos="fade-up">{displayName || 'N/A'}</h1>
                        <h3 className="h3 mb-4">{email || 'N/A'}</h3>
                        <div className="max-w-xs mx-auto sm:max-w-none sm:flex sm:justify-center">
                            <div data-aos="fade-up" data-aos-delay="400">
                                <Dialog>
                                    <DialogTrigger>
                                        <div className="btn text-white bg-purple-600 hover:bg-purple-700 w-full mb-4 sm:w-auto sm:mb-0">
                                            Đổi mật khẩu
                                        </div>
                                    </DialogTrigger>
                                    <DialogContent className='bg-black'>
                                        ...
                                        <div className="flex flex-col gap-4">
                                            <input type="password" placeholder="Mật khẩu mới" className='p-2 text-black'
                                                value={newPassword} onChange={e => setNewPassword(e.target.value)} />
                                            <input type="password" placeholder="Xác nhận mật khẩu" className='p-2 text-black'
                                                value={confirmPassword} onChange={e => setConfirmPassword(e.target.value)} />
                                        </div>
                                        <DialogFooter>
                                            <Button type="submit" variant={'outline'} onClick={handleCreateNewPassword}>Cập nhật</Button>
                                        </DialogFooter>
                                    </DialogContent>
                                </Dialog>
                            </div>
                            <div data-aos="fade-up" data-aos-delay="600">
                                <Dialog>
                                    <DialogTrigger>
                                        <div className="btn text-white bg-gray-700 hover:bg-gray-800 w-full sm:w-auto sm:ml-4">
                                    ...
                                    <DialogContent className='bg-black'>
                                        <DialogHeader className="flex flex-row items-center justify-between">
                                            ...
                                            <DialogTitle>Cập nhật tên và ảnh đại diện</DialogTitle>
                                        </DialogHeader>
                                        <div className="flex flex-col gap-4">
                                            <input type="text" placeholder="Tên hiển thị" className='p-2 text-black'
                                                value={newDisplayName} onChange={e => setNewDisplayName(e.target.value)} />
                                            <input type="file" accept="image/*"
                                                onChange={handleImageChange} className="mt-2 text-white" />
                                        </div>
                                        <DialogFooter>
                                            <Button type="submit" variant={'outline'} onClick={handleUpdateProfile}>Cập nhật</Button>
                                        </DialogFooter>
                                    </DialogContent>
                                </Dialog>
                            </div>
